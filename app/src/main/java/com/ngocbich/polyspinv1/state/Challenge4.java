...
public class Challenge4 extends GameState {
    ...
    public Challenge4(GameStateManager gsm, BitmapImage bitmapImage) {
            ...
            if (starItem_manager.isPlayerColliding(player.getRect())) {
                for (StarItem item : starItem_manager.getStarItems()) {
                    if (item.isColliding(player.getRect())) {
                        // starItem_manager.getStarItems().remove(item);
    ...
    public void draw(Canvas canvas) {
        if (ChallengeState.failed==false) {
            ...
            //   canvas.drawBitmap(bgstar,null,new Rect(0,0,Constants.SCREEN_WIDTH,Constants.SCREEN_WIDTH),new Paint());
            spinnie.draw(canvas);
            player.draw(canvas);
            circle_manager.draw(canvas);
            starItem_manager.draw(canvas);

            canvas.drawBitmap(star,null,new Rect(50,140,100,190),new Paint());
            canvas.drawText("" + countStar, 130, 100 + paint.descent() - paint.ascent(), paint);
        }
    }
